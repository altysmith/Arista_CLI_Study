...
# Run from the repository directory even when launched from Finder.
cd "$(dirname "$0")" || exit 1
...
for candidate in "${python_candidates[@]}"; do
  ...
  if "$python_cmd" -c 'import sys; raise SystemExit(sys.version_info < (3, 11))' 2>/dev/null; then
    "$python_cmd" -m arista_sim
    exit_code=$?
    if (( exit_code != 0 )) && [[ -t 0 ]]; then
      read -k 1 "?The simulator exited with an error. Press any key to close..."
      print
    fi
    exit "$exit_code"
  fi
