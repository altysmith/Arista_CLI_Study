...
class CommandReferenceTests(unittest.TestCase):
    ...
    def test_exercise_families_evaluate_and_select_deterministically(self):
        ...
        self.assertTrue(evaluate_attempt("vlan-trunk-mismatch", "missing-vlan-30", "switchport trunk allowed vlan add 30")["correct"])
        self.assertTrue(evaluate_attempt("acl-first-match", "deny-before-permit", "denied")["correct"])
        self.assertTrue(evaluate_attempt("eos-config-state", "inspect-saved", "show startup-config")["correct"])
        self.assertTrue(evaluate_attempt("lacp-compatibility", "active-passive", "yes")["correct"])
        self.assertTrue(evaluate_attempt("static-default-route", "gateway-198-51-100-1", "ip route 0.0.0.0/0 198.51.100.1")["correct"])
        self.assertTrue(evaluate_attempt("ospf-first-failed-dependency", "full-route-missing", "advertisement and route selection")["correct"])
